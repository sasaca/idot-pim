...
const VALID_DOMAINS = new Set(FORM_DOMAINS.map((d) => d.value));
function domainLabel(value) {
  const d = FORM_DOMAINS.find((x) => x.value === value);
...
function normalize(schema) {
  ...
  schema.domain = VALID_DOMAINS.has(schema.domain) ? schema.domain : 'general';
  return schema;
}
...
module.exports = {
  FIELD_TYPES, VALID_TYPES,
  FORM_DOMAINS, VALID_DOMAINS, domainLabel,
  normalize, renderField, renderForm, esc,
};
