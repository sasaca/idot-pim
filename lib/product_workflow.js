...
const TRANSITIONS = Object.freeze({
  ...
  //
  // Production — never confirms on its own; it always passes the baton to
  // R&D's Packaging Materials form.
  submit_production: {
    from: [STATES.PENDING_PRODUCTION_AND_ANALYSIS],
    to:   STATES.PENDING_PRODUCTION_AND_ANALYSIS,
    allowed_roles: [ROLES.RND_TEAM, ROLES.ADMIN],
    label: 'Submit Production (next: Packaging Materials)',
  },
  // Packaging Materials — partial when Marketing's competitor analysis
  // is still pending.
  submit_packaging_partial: {
    from: [STATES.PENDING_PRODUCTION_AND_ANALYSIS],
    to:   STATES.PENDING_PRODUCTION_AND_ANALYSIS,
    allowed_roles: [ROLES.RND_TEAM, ROLES.ADMIN],
    label: 'Submit Packaging Materials (waiting on Marketing)',
  },
  // Packaging Materials — final when Marketing's competitor analysis is
  // already complete.
  submit_packaging_final: {
    from: [STATES.PENDING_PRODUCTION_AND_ANALYSIS],
    to:   STATES.CONFIRMED,
    allowed_roles: [ROLES.RND_TEAM, ROLES.ADMIN],
    label: 'Submit Packaging Materials (final — confirms request)',
  },
  // Competitor Analysis — partial when R&D's track (production AND
  // packaging) isn't fully done yet.
  ...
  // Competitor Analysis — final when both R&D forms are already complete.
