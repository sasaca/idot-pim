...
const STATES = Object.freeze({
  ...
  PENDING_RND_AND_QUALITY_DIRECTORS:  'PENDING_RND_AND_QUALITY_DIRECTORS',
  PENDING_LEGAL_AND_MDM:              'PENDING_LEGAL_AND_MDM',
...
const STATE_LABELS = Object.freeze({
  ...
  PENDING_RND_AND_QUALITY_DIRECTORS: 'Pending R&D Director and Quality Director approvals',
  PENDING_LEGAL_AND_MDM:             'Pending Legal certification and MDM material review',
...
const TRANSITIONS = Object.freeze({
  ...
  // R&D's last working form. Hands off to the dual director approval.
  submit_bom: {
    from: [STATES.PENDING_BOM],
    to:   STATES.PENDING_RND_AND_QUALITY_DIRECTORS,
    allowed_roles: [ROLES.RND_TEAM, ROLES.ADMIN],
    label: 'Submit BOM (sends to R&D Director + Quality Director)',
  },

  // R&D Director — runs in parallel with Quality Director.
  rnd_dir_approve_partial: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.PENDING_RND_AND_QUALITY_DIRECTORS,
    allowed_roles: [ROLES.RND_DIRECTOR, ROLES.ADMIN],
    label: 'R&D Director approve (waiting on Quality Director)',
  },
  rnd_dir_approve_final: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.PENDING_LEGAL_AND_MDM,
    allowed_roles: [ROLES.RND_DIRECTOR, ROLES.ADMIN],
    label: 'R&D Director approve (final — opens Legal + MDM stage)',
  },
  rnd_dir_reject: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.REJECTED,
    allowed_roles: [ROLES.RND_DIRECTOR, ROLES.ADMIN],
    label: 'R&D Director reject',
  },
  rnd_dir_request_info: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.NEEDS_INFO,
    allowed_roles: [ROLES.RND_DIRECTOR, ROLES.ADMIN],
    label: 'R&D Director request more information',
  },

  // Quality Director — runs in parallel with R&D Director.
  quality_dir_approve_partial: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.PENDING_RND_AND_QUALITY_DIRECTORS,
    allowed_roles: [ROLES.QUALITY_DIRECTOR, ROLES.ADMIN],
    label: 'Quality Director approve (waiting on R&D Director)',
  },
  quality_dir_approve_final: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.PENDING_LEGAL_AND_MDM,
    allowed_roles: [ROLES.QUALITY_DIRECTOR, ROLES.ADMIN],
    label: 'Quality Director approve (final — opens Legal + MDM stage)',
  },
  quality_dir_reject: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.REJECTED,
    allowed_roles: [ROLES.QUALITY_DIRECTOR, ROLES.ADMIN],
    label: 'Quality Director reject',
  },
  quality_dir_request_info: {
    from: [STATES.PENDING_RND_AND_QUALITY_DIRECTORS],
    to:   STATES.NEEDS_INFO,
    allowed_roles: [ROLES.QUALITY_DIRECTOR, ROLES.ADMIN],
    label: 'Quality Director request more information',
  },

  // Post-director parallel stage. Legal and MDM run independently; the
  // request only confirms once both have submitted. The route handler picks
  // the "_final" variant when this submit closes the LAST outstanding flag.
  submit_legal_partial: {
    from: [STATES.PENDING_LEGAL_AND_MDM],
    to:   STATES.PENDING_LEGAL_AND_MDM,
    allowed_roles: [ROLES.LEGAL, ROLES.ADMIN],
    label: 'Legal submit (waiting on MDM)',
  },
  submit_legal_final: {
    from: [STATES.PENDING_LEGAL_AND_MDM],
    to:   STATES.CONFIRMED,
    allowed_roles: [ROLES.LEGAL, ROLES.ADMIN],
    label: 'Legal submit (final — confirms request)',
  },
  submit_mdm_partial: {
    from: [STATES.PENDING_LEGAL_AND_MDM],
    to:   STATES.PENDING_LEGAL_AND_MDM,
    allowed_roles: [ROLES.MDM_TEAM, ROLES.ADMIN],
    label: 'MDM submit (waiting on Legal)',
  },
  submit_mdm_final: {
    from: [STATES.PENDING_LEGAL_AND_MDM],
    to:   STATES.CONFIRMED,
    allowed_roles: [ROLES.MDM_TEAM, ROLES.ADMIN],
    label: 'MDM submit (final — confirms request)',
  },
